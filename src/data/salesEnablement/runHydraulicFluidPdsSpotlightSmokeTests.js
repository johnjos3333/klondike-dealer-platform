/**
 * Phase 76.0 — Node smoke tests for HYDRAULIC_FLUID_PDS_SPOTLIGHT_MAP (products + category spotlights).
 * Run: npm run test:hydraulic-fluid-pds-spotlights
 */
...
function verifyProduct(p, index) {
  const failures = [];
  const loc = `HYDRAULIC_FLUID_PDS_SPOTLIGHT_MAP.products[${index}]`;
  ...
  if (String(p.categoryId ?? "").trim() !== "hydraulic_fluids") {
    failures.push(`${loc} (${pid || "?"}) categoryId must be "hydraulic_fluids"`);
  }
  ...
  if (!Array.isArray(p.productNumbers)) {
    failures.push(`${loc} (${pid || "?"}) productNumbers must be an array`);
  }
  if (!Array.isArray(p.pdsSafeNotes)) {
    failures.push(`${loc} (${pid || "?"}) pdsSafeNotes must be an array`);
  }
...
function verifyCategorySpotlight(c, index, productIds) {
  const failures = [];
  const loc = `HYDRAULIC_FLUID_PDS_SPOTLIGHT_MAP.categorySpotlights[${index}]`;
...
const products = Array.isArray(HYDRAULIC_FLUID_PDS_SPOTLIGHT_MAP?.products)
  ...
  : [];
const categorySpotlights = Array.isArray(HYDRAULIC_FLUID_PDS_SPOTLIGHT_MAP?.categorySpotlights)
...
if (products.length === 0) {
  results.push({ label: "(no products)", failures: ["HYDRAULIC_FLUID_PDS_SPOTLIGHT_MAP.products empty"] });
}
...
console.log("\nHYDRAULIC_FLUID_PDS_SPOTLIGHT_MAP smoke tests\n" + "=".repeat(56));
...
if (failCount > 0) {
  const detail = results
    ...
    .join("\n");
  throw new Error(`HYDRAULIC_FLUID_PDS_SPOTLIGHT_MAP smoke tests failed:\n${detail}`);
}
